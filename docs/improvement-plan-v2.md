...
| 시각화 | 디지털 드레이핑 시뮬레이션 (MediaPipe) | ✅ 완료 (2026-03-06) |
...
| 결과 | 시즌 스토리텔링 (A3) | ✅ 완료 |
| 교육 | 3축 교육 카드 (B5) | ✅ 완료 |
| AI | AI 점수 리밸런스 (temp/clarity 대칭화) | ✅ 완료 (2026-03-06) |
...
### C1. 디지털 드레이핑 시뮬레이션 ✅ 완료 (2026-03-06)

**MediaPipe FaceLandmarker 기반 고급 드레이핑 구현.**

#### 구현 방식 (접근법 2: MediaPipe Face Mesh)

- MediaPipe FaceLandmarker CDN Lazy load (사용자가 결과 화면 볼 때만)
- Face Oval 36개 랜드마크 중 하단 턱선 20개 추출
- 턱선 폭 ×1.4, 얼굴 높이 ×0.7 하방 확장 → 목/가슴 사다리꼴
- Canvas clip() + fill() 반투명 (0.55) 오버레이 + 광택 그라디언트
- 랜드마크 1회 감지 후 캐싱 (색상 변경 시 재감지 안함)
- 2탭: "✅ Nên mặc" (palette 12색) / "❌ Nên tránh" (avoid 6색)
- ◀▶ 네비게이션 + 모바일 스와이프 + 300ms ease-out 전환
- 에러 처리: CDN 실패, 얼굴 미감지, 이미지 too small

#### 수정 파일
- `js/draping.js` (신규 ~280줄): 전체 드레이핑 로직
- `index.html`: 드레이핑 카드 + 퀴즈용 CTA 카드
- `css/screens.css`: 드레이핑 UI 스타일 ~90줄
- `js/result.js`: `initDraping()` import 및 호출
- `js/app.js`: window 바인딩 4개
...
### Phase 2.8: 드레이핑 + AI 리밸런스 ✅ 완료 (2026-03-06)

| 순서 | 작업 | 수정 파일 | 상태 |
|------|------|----------|------|
| 11 | C1. 디지털 드레이핑 (MediaPipe Face Mesh) | draping.js(신규), index.html, screens.css, result.js, app.js | ✅ 완료 |
| 12 | AI 점수 리밸런스 (temp/clarity 대칭화) | ai-analysis.js | ✅ 완료 |
| — | 인트로 프라이버시 안내문 추가 | index.html, screens.css | ✅ 완료 |

### Phase 3: 고급 (향후)

| 순서 | 작업 | 비고 |
|------|------|------|
| 13 | C2. AI 엔진 업그레이드 (CIELAB + Von Kries) | 색 보정 고도화 |
| 14 | C3. 이메일 소프트 게이트 | 수익화 |
| 15 | C4. 옷장 컬러 체크 | 장기 차별화 |
| 16 | GA4 실제 태그 연결 | 추적 |
